...
map ,p :call scnvim#sclang#send("Song.play")<CR>
map zp m`:call SelectPart()<CR>zl,,``
map ,ii yaw:call scnvim#sclang#send("Song.current.<C-r>".play")<CR>
map ,, :call scnvim#sclang#send("Part.play")<CR>
map ,sc :call scnvim#sclang#send("")<left><Left>
nmap ,st :SCNvimStart<CR>
nmap ,sp :SCNvimStop<CR>
map zk <ESC>:w<ENTER>:SCNvimRecompile<ENTER>:silent!open -a iTerm<RETURN>
nmap zx :call scnvim#sclang#send("~myFree.()")<CR>
nmap <LEADER>. <Plug>(scnvim-send-block)
"vmap <LEADER>l <Plug>(scnvim-send-selection)
vmap zl <Plug>(scnvim-send-selection)
nmap zl <Plug>(scnvim-send-line)
...
function! LookupSynthDef(def)
	exe command = "vim /\\" . a:name . "/j ~/tank/super/SynthDefLibrary/* | cw"
	"exe command
endfunction
